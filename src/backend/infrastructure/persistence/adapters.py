...
class SqlUsersAdapter:
    """Реализация UsersPort: SQL, маппинг строк в доменную сущность
    ...
    async def add(self, user: User) -> User:
        stmt = sa.insert(users_table).values(**_user_to_row(user)).returning(*users_table.c)
        try:
...
class SqlPostsAdapter:
    ...
    async def add(self, post: Post) -> Post:
        stmt = sa.insert(posts_table).values(**_post_to_row(post)).returning(*posts_table.c)
        try:
            row = (await self._session.execute(stmt)).mappings().one()
        except IntegrityError as exc:
...
class SqlCommentsAdapter:
    ...
    async def add(self, comment: Comment) -> Comment:
        stmt = (
            ...
            .values(**_comment_to_row(comment))
...
def _comment_to_row(comment: Comment) -> dict[str, Any]:
    return {
        "id": comment.id,
